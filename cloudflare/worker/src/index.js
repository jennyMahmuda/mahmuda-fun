...
async function handleAdminLogin(request, env, origin) {
  ...
  if (!username || !password || !configuredUsername || !env.REVIEWS_DB) return json({ error: 'Admin login is not configured' }, 503, corsHeaders(origin));
  ...
  const user = await env.REVIEWS_DB.prepare('SELECT id, password_hash AS passwordHash FROM users WHERE email = ? AND is_admin = 1').bind(configuredUsername).first();
  const ok = await verifyPassword(password, user ? user.passwordHash : DUMMY_PASSWORD_HASH);
  await recordLoginAttempt(env, username, ipHash, ok && !!user);
  if (!user || username !== configuredUsername || !ok) return json({ error: 'Incorrect admin username or password' }, 401, corsHeaders(origin));
  const sessionToken = generateToken();
